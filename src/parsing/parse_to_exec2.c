...
t_tab_parse *init_tab_parse(t_lexer **deb_lexer)
{
	...
	while (tab_parse->nb_cmd > y)
	{
		tab_parse[y].tab_args =ft_malloc(sizeof(char **) *  len_lexer(deb_lexer) + 1);
		tab_parse[y].infile =ft_malloc(sizeof(char **) * i);
		tab_parse[y].outfile =ft_malloc(sizeof(char **) * i);
		tab_parse[y].inredir =ft_malloc(sizeof(char **) * i);
		tab_parse[y].outredir =ft_malloc(sizeof(char **) * i);
		y++;
	}
	...
	// 		tab_parse[y].infile[i] =ft_malloc(sizeof(char *) * tab_parse->nb_redir);
